	...
	public Bill(Phong phong, ManagerUI view) {
		...
		NutXacnhan.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				xuatfile(); // Xuất file hóa đơn

				String maDpStr = TMadp.getText();
				if (maDpStr == null || maDpStr.trim().isEmpty()) {
					JOptionPane.showMessageDialog(null, "Mã đặt phòng không được để trống!", "Lỗi", JOptionPane.ERROR_MESSAGE);
					return;
				}

				try {
					int madp = Integer.parseInt(maDpStr);

					// Gọi phương thức lưu thông tin vào CSDL và các thao tác khác
					// Giả sử thongtintraphong trả về true nếu thành công, false nếu thất bại
					boolean luuThanhCong = thongtintraphong(madp, view); // 'view' ở đây là instance của ManagerUI

					if (luuThanhCong) {
						dispose(); // Đóng cửa sổ Bill

						// Cập nhật trạng thái của đối tượng Phong trong bộ nhớ
						if (phong != null) {
							phong.setTrangThai(TrangThaiPhong.TRONG);
						}

						// YÊU CẦU ManagerUI CẬP NHẬT SAU KHI TRẢ PHÒNG
						// (bao gồm cả việc gọi xoaform() cho pn_DanghoatdongQL tương ứng)
						if (view != null && phong != null) {
							view.capNhatSauKhiTraPhong(phong.getId()); // << GỌI HÀM CẬP NHẬT Ở ĐÂY
						}

						// Chuyển về màn hình sơ đồ phòng
						if (view != null && view.pn_hoatdong != null && view.cardhd != null) {
							view.cardhd.show(view.pn_hoatdong, "sơ đồ phòng");
						}
					} else {
						// Thông báo nếu việc lưu thông tin thất bại
						JOptionPane.showMessageDialog(null, "Lưu thông tin trả phòng thất bại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
					}
				} catch (NumberFormatException nfe) {
					JOptionPane.showMessageDialog(null, "Mã đặt phòng không hợp lệ: '" + maDpStr + "'", "Lỗi Định Dạng", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
...
// Lấy chuỗi ngày giờ vào phòng từ JTextField TNgayvao (bạn cần đảm bảo TNgayvao đã được khởi tạo và có giá trị)
		String gioVaoPhongString = ""; // Khởi tạo rỗng
		if (TNgayvao != null) { // Kiểm tra TNgayvao có null không (TNgayvao là JTextField của bạn)
			gioVaoPhongString = TNgayvao.getText();
		} else {
			// Xử lý trường hợp TNgayvao là null, có thể gán giá trị mặc định hoặc báo lỗi
			System.err.println("Lỗi: JTextField TNgayvao chưa được khởi tạo.");
		}


		String timestamp; // Biến này sẽ lưu timestamp đã định dạng từ giờ vào phòng

// Định dạng cho chuỗi ngày giờ vào phòng đầu vào (ví dụ: "08:30:00 29/05/2025")
		DateTimeFormatter inputFormatter = DateTimeFormatter.ofPattern("HH:mm:ss dd/MM/yyyy");
// Định dạng cho timestamp trong tên file (ví dụ: "20250529_083000")
		DateTimeFormatter outputDtf = DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss");

		try {
			if (gioVaoPhongString == null || gioVaoPhongString.trim().isEmpty()) {
				throw new DateTimeParseException("Chuỗi ngày giờ vào phòng rỗng hoặc null", "", 0);
			}
			LocalDateTime checkInDateTime = LocalDateTime.parse(gioVaoPhongString, inputFormatter);
			timestamp = outputDtf.format(checkInDateTime);

		} catch (DateTimeParseException e) {
			// Xử lý lỗi nếu định dạng của gioVaoPhongString không đúng
			e.printStackTrace();
			JOptionPane.showMessageDialog(null,
					"Lỗi định dạng Ngày & Giờ vào phòng: '" + gioVaoPhongString + "'.\n" +
							"Định dạng dự kiến: HH:mm:ss dd/MM/yyyy.\n" +
							"Sẽ sử dụng thời gian hiện tại để tạo tên file.",
					"Lỗi Định Dạng Ngày Giờ", JOptionPane.ERROR_MESSAGE);

			// Dự phòng: Sử dụng thời gian hiện tại nếu có lỗi phân tích cú pháp
			LocalDateTime now = LocalDateTime.now();
			timestamp = outputDtf.format(now);
		}

// Tạo tên file với timestamp (từ giờ vào phòng) và thư mục đích
		String directoryPath = "D:\\KyHai2024.2025\\DoAnCoSo1\\DACS1\\Bill_Homestay";
		File directory = new File(directoryPath);

// Tạo thư mục nếu chưa tồn tại
		if (!directory.exists()) {
			boolean created = directory.mkdirs(); // Sử dụng mkdirs() để tạo cả các thư mục cha nếu cần
			if (!created) {
				System.err.println("Không thể tạo thư mục: " + directoryPath);
				// Có thể thêm xử lý lỗi ở đây nếu không tạo được thư mục
			}
		}
