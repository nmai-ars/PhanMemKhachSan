...
public class pn_DanghoatdongQL extends JPanel {
	...
	private JButton btXoaDichVuDatSau;
	private JButton btXoaDichVuDatTruoc;
	...
	public pn_DanghoatdongQL(Phong phong, ManagerUI view) {
		...
		btXoaDichVuDatTruoc = new JButton("Xóa");
		btXoaDichVuDatTruoc.setBackground(new Color(255, 182, 193)); // Màu hồng nhạt
		btXoaDichVuDatTruoc.setFont(new Font("Tahoma", Font.PLAIN, 12)); // Font nhỏ hơn một chút
		btXoaDichVuDatTruoc.setBounds(33, 500, 160, 30); // Vị trí dưới panel_2
		panel.add(btXoaDichVuDatTruoc);

		btXoaDichVuDatTruoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (table_1 == null) return; // Kiểm tra table_1
				int selectedRow = table_1.getSelectedRow();
				if (selectedRow != -1) {
					int confirm = JOptionPane.showConfirmDialog(panel, // parentComponent
							"Bạn có chắc muốn xóa dịch vụ đặt trước này không?",
							"Xác nhận xóa", JOptionPane.YES_NO_OPTION);
					if (confirm == JOptionPane.YES_OPTION) {
						db.removeRow(selectedRow);
					}
				} else {
					JOptionPane.showMessageDialog(panel, // parentComponent
							"Vui lòng chọn một dịch vụ trong bảng 'Dịch vụ đặt trước' để xóa.",
							"Chưa chọn dịch vụ", JOptionPane.WARNING_MESSAGE);
				}
			}
		});
		...
		btXoaDichVuDatSau = new JButton("Xóa");
		btXoaDichVuDatSau.setBackground(new Color(255, 182, 193)); // Màu hồng nhạt
		btXoaDichVuDatSau.setFont(new Font("Tahoma", Font.PLAIN, 14));
		btXoaDichVuDatSau.setBounds(149, 163, 129, 51); // Vị trí cạnh nút "Đặt"
		panel_5.add(btXoaDichVuDatSau);

		btXoaDichVuDatSau.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (table2 == null) return; // Kiểm tra table2 đã được khởi tạo chưa
				int selectedRow = table2.getSelectedRow();
				if (selectedRow != -1) {
					int confirm = JOptionPane.showConfirmDialog(pnOrderDv, // parentComponent
							"Bạn có chắc muốn xóa dịch vụ đã chọn này không?",
							"Xác nhận xóa", JOptionPane.YES_NO_OPTION);
					if (confirm == JOptionPane.YES_OPTION) {
						dbsau.removeRow(selectedRow);
					}
				} else {
					JOptionPane.showMessageDialog(pnOrderDv, // parentComponent
							"Vui lòng chọn một dịch vụ trong bảng 'Dịch vụ đặt thêm' để xóa.",
							"Chưa chọn dịch vụ", JOptionPane.WARNING_MESSAGE);
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
	            ...
	            for (int row = 0; row < db.getRowCount(); row++) {
	                ...
	                bill.db.addRow(new Object[] {
	                	col1Value, col2Value, 1, phantichgia(gia)
	                });
				}
